...
class PatientData extends BasePatientComponent
{
    ...
    public function syncPersonDataFromEHealth(): void
    {
        ...
        $authorizeWith = collect($this->authenticationMethods)->firstWhere('type', 'OTP') ??
            collect($this->authenticationMethods)->firstWhere('type', 'THIRD_PERSON with OTP') ??
            collect($this->authenticationMethods)->firstWhere('type', 'OFFLINE') ?? null;
        ...
        $payloadData = [
            ...
            'authorize_with' => $authorizeWith['id']
        ];
        ...
        if (!$responseCode && empty($links) && !empty($responseData['is_verified'])) {
            ...
            DB::transaction(function () use ($authorizeWith, $links, $job) {
                ...
                foreach ($links as $link) {
                    $approval = MERepository::approval()->create();
