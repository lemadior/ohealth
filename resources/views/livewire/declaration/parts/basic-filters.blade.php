    ...
    @if(legalEntity()->legators->isNotEmpty())
        <div class="relative">
            ...
            <input type="text"
                ...
                class="input peer w-full cursor-pointer text-gray-500 dark:text-gray-400"
                @click="open1 = !open1"
                :value="reorganizedDeclarations.length ? reorganizedDeclarations.map(status => status === @js(ReorganizedStatus::TO_BE_RESIGNED->value) ?  @js(__('declarations.to_be_resigned')) : (status === @js(ReorganizedStatus::RESIGNED->value) ? @js(__('declarations.resigned')) : status)).join(', ') : ''"
