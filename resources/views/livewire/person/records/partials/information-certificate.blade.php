    ...
    <div x-transition @click="showCertificate = false" class="modal-wrapper">
        ...
        >
            <div id="certificate-print-area" class="p-8">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">{{ __('preperson.label_single') }}</p>
                ...
                <div class="flex items-center justify-center mb-6">
                    @php
                        ...
                        $len = strlen($preperson->uuid);
                        for ($i = 0; $i < $len; $i++) {
                            $char = $preperson->uuid[$i];
                            $val = ord($char) - 32;
                            if ($val < 0 || $val > 95) {
                                $val = 0;
                            }
                    ...
                    @endphp
                    <svg viewBox="0 0 {{ $x }} 80" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg"
                         class="w-full"
                         style="width: 100%; height: 2cm; min-width: 9cm; min-height: 1.2cm; display: block; margin: 0 auto;">
                        {!! $svgContent !!}
                    </svg>
                </div>
