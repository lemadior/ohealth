    ...
    <div class="form-row-2 mt-5">
        <div class="form-group group">
            <select id="intent"
                    ...
                    :disabled="$isReadOnly ?? false"
            >
                <option value="order" @if(($isReadOnly ?? false) && $intent === 'order') selected @endif>{{ __('care-plan.order') }}</option>
                <option value="proposal" @if(($isReadOnly ?? false) && $intent === 'proposal') selected @endif>{{ __('care-plan.proposal') }}</option>
                <option value="plan" @if(($isReadOnly ?? false) && $intent === 'plan') selected @endif>{{ __('care-plan.plan') }}</option>
            </select>
